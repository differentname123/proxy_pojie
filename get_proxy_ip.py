...
def check_little_empire(ip_port):
    ...
    while result == False:
        try:
            ...
            if "ios" in str(response.content):
                result = True
...
def check_proxy(ip_port):
    ...
    while result == False:
        try:
            ...
            response = requests.get(url, headers=headers, proxies=proxy, timeout= 5)
            if "全球领先的中文搜索引擎" in str(response.content, 'utf-8'):
                result = True
                break
        except Exception as e:
            print ("check_ip_port error %s" % traceback.format_exc())
...
def get_proxy_ip():
    ...
    try:
        ...
        while count_index < len(data_arr):
            ...
            if len(ip_result) == 1:
                count_index += 1
                port = data_arr[count_index].split(">")[1].split("<")[0].strip()
                ip_data = ip_result[0]
                ip_port = "%s:%s\n" % (ip_data, port)
                ip_port_list.append(ip_port)
            count_index += 1
    ...
    try:
        ...
        while count_index < len(data_arr):
            ...
            if len(ip_result) == 1:
                count_index += 1
                port = data_arr[count_index].split("</td>")[0].split(">")[1].strip()
                ip_data = ip_result[0]
                ip_port = "%s:%s\n" % (ip_data, port)
                ip_port_list.append(ip_port)
            count_index += 1
